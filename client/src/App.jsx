...
import { LayoutDashboard, Users, Calendar, Music, Menu, X, ChevronRight, CalendarDays, BarChart3, RefreshCw, Settings, LogOut } from 'lucide-react';
...
export default function App() {
  ...
  const allNavItems = [
    { name: 'Dashboard', path: '/dashboard', icon: LayoutDashboard, perm: true },
    { name: 'Agenda', path: '/agenda', icon: CalendarDays, perm: permissions.viewAgenda },
    { name: 'Fornecedores', path: '/fornecedores', icon: Users, perm: permissions.viewFornecedores },
    { name: 'Eventos', path: '/eventos', icon: Calendar, perm: permissions.viewEventos },
    { name: 'Relatórios', path: '/relatorios', icon: BarChart3, perm: permissions.viewRelatorios },
    { name: 'Sincronização', path: '/sincronizacao', icon: RefreshCw, perm: permissions.viewSincronizacao },
    { name: 'Admin', path: '/admin', icon: Settings, perm: isAdmin },
  ];

  const navItems = allNavItems.filter(item => item.perm);

  const getPageTitle = () => {
    const item = allNavItems.find(i => i.path === location.pathname);
    return item?.name || 'Gestão Musical';
  };
  ...
  const roleLabel = isAdmin ? 'Admin' : 'Visualizador';
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans">
        ...
        <div className="md:hidden bg-emerald-950 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-md">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-300/80">{user?.name} • {roleLabel}</p>
            </div>
          </div>
        ...
        <aside className="hidden md:flex w-64 bg-emerald-900 text-white flex-col justify-between fixed inset-y-0 left-0 z-40 shadow-xl">
          ...
          <div className="p-4 border-t border-emerald-800/80 bg-emerald-950/40">
            ...
            <p className="text-[10px] text-emerald-300/50">v1.3.0 • Terra Parque Resort</p>
          </div>
        </aside>
        ...
        <div className="flex-1 flex flex-col min-w-0 md:ml-64">
          ...
          <header className="hidden md:flex bg-white border-b border-slate-200/80 px-6 py-4 items-center justify-between shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Painel de gerenciamento • Terra Parque Eco Resort</p>
            </div>
          ...
          <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 lg:p-8 bg-slate-50 pb-20 md:pb-8">
            <Routes>
              ...
              <Route path="/agenda" element={permissions.viewAgenda ? <Agenda /> : <Navigate to="/dashboard" replace />} />
              <Route path="/fornecedores" element={permissions.viewFornecedores ? <Fornecedores /> : <Navigate to="/dashboard" replace />} />
              <Route path="/eventos" element={permissions.viewEventos ? <Eventos /> : <Navigate to="/dashboard" replace />} />
              <Route path="/relatorios" element={permissions.viewRelatorios ? <Relatorios /> : <Navigate to="/dashboard" replace />} />
              <Route path="/sincronizacao" element={permissions.viewSincronizacao ? <Sincronizacao /> : <Navigate to="/dashboard" replace />} />
